...
class _census extends CI_Model
{
	...
	{
		$isAdded = false;
		$required = array('teachername__teachers', 'teacherid', 'censusstart', 'censusend', 'averageworkload');
		...
		{
			$details = $passed['data'];
			$censusId = $this->_query_reader->add_data('add_census_data', array('teacher_id'=>$details['teacherid'], 'start_date'=>format_date($details['censusstart'], 'YYYY-MM-DD', ''), 'end_date'=>format_date($details['censusend'], 'YYYY-MM-DD', ''), 'weekly_workload_average'=>$details['averageworkload'], 'added_by'=>$this->native_session->get('__user_id') ));
