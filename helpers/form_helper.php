...
{echo "<BR><BR>IN HELPER!!";
	...
	foreach($data AS $key=>$value)
	{
		...
		if(!is_array($value))
		{
			...
			{
				echo "<BR>VALUE: ".$value. " DISALLOWED: "; print_r($disallowChars);
        if(!empty($value) && (0 < count(array_intersect(str_split(strtolower($value)), $disallowChars)))) $msg = "WARNING: The submitted data contains disallowed characters.";
